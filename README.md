...
- B-05 自由辩论：全员完成首轮发言后服务端自动判定并实时广播进入（管理员也可手动推进）
- B-06 总结陈词：管理员推进阶段，每方仅限一条总结发言（🎤 标记）
- B-08 阶段状态机：正式轮辩 → 自由辩论 → 总结陈词（单向推进，含审计与广播）
...
**游客模式（无需注册即可传播）**
- 大厅/辩论档案（匿名实时只读观战）/排行榜/众创/公开主页/重辩链 均未登录可浏览
- 互动（发言/投票/聊天/支持/情绪/举报）引导登录；服务端匿名连接仅开放只读事件

**性能与可靠性**
- SQLite WAL + busy_timeout（读写并行、锁等待）；前端路由级代码分割（首屏 gzip ~97KB）
...
- 🧬 重辩链聚合页：同一辩题历次交锋的胜负/票数/支持率并排对比（已结束场次可点“历次交锋对比”）
- 🏆 排行榜（C5）：段位榜 + 活跃榜（30 天），附我的名次
- 💡 辩题众创（D-10）：提案 + 一人一票（可取消）+ 管理员采纳
- 🌐 公开个人主页（C4）：任意辩手战绩与辩论足迹
- 🖼️ 金句分享卡：一键生成 1200×630 PNG（I-04）
...
# 生成：用户1~用户12（密码 123456）
#      辩题1 完整 8 人赛（AI报告/辩论树/金句卡/精选）
#      辩题2 1v1 平局 + 重辩胜场（重辩链对比）
#      辩题3 进行中 / 辩题4 报名中 / 辩题5 待审核
#      众创提案 ×3 · 段位梯度校准
# 幂等：再次运行自动清理重建；不影响种子内容
```
...
# 后端回归测试（67 用例：服务层 6 套件 + 真实 HTTP 集成套件）
# 覆盖认证/封禁/1v1/冷静期/结算/报告/辩论树/敏感词/举报/审计/重辩链/检索/相关推荐/运营精选/安全纵深/阶段/众创/榜单
cd server && npm test
